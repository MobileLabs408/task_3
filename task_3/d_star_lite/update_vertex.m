...
function [U, push] = update_vertex(U, start_position, position, g, rhs, k_m, push)
    ...
    if((g(node.position(1), node.position(2)) ~= rhs(node.position(1), node.position(2))) && (any(arrayfun(@(x) isequal(x.position, node.position), U))))
        % Update by replacing
        U(find(arrayfun(@(x) isequal(x.position, node.position), U), 1)) = node;

    % Wrong g value but node is not in U, insert node
    elseif((g(node.position(1), node.position(2)) ~= rhs(node.position(1), node.position(2))) && (~any(arrayfun(@(x) isequal(x.position, node.position), U))))
        % Push node
        U = [U; node];
        push = push + 1;
